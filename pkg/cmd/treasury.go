...
var treasuryRetrieveStatements = cli.Command{
...
func handleTreasuryRetrieveStatements(ctx context.Context, cmd *cli.Command) error {
	client := mercury.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	params := mercury.TreasuryGetStatementsParams{}
	...
	if format == "raw" {
		...
		obj := gjson.ParseBytes(res)
		return ShowJSON(os.Stdout, "treasury retrieve-statements", obj, format, transform)
	} else {
		iter := client.Treasury.GetStatementsAutoPaging(
			ctx,
			cmd.Value("treasury-id").(string),
			params,
			options...,
